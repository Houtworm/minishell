...
/*   Updated: 2023/10/21 11:53:40 by houtworm         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_nextsubanyof(t_globs *globs, int si, int ri, int gi)
{
	...
	if (globs->sdir[si][gi] == '\0' && globs->tmpsdir[si][ri] == '\0')
		if (globs->temptype == DT_DIR || !globs->sdir[si + 1])
			return (1);
	if (globs->sdir[si][gi - 1] != '\\' && globs->sdir[si][gi] && ft_strchr("*?[", globs->sdir[si][gi]))
...
int	ft_firstsubanyof(t_globs *globs, struct dirent *dirents, int si, int gi)
{
	...
	if (ft_strchr(globs->anyof, dirents->d_name[ri]))
	{
		...
		if (dirents->d_name[ri] == '\0' && globs->sdir[si][gi] == '\0' && (dirents->d_type == DT_DIR || !globs->sdir[si + 1]))
		{
